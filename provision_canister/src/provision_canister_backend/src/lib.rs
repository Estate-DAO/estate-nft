...
use guard::caller_is_authorized_principal;
...
use state::{
    AdditionalMetadata, ApprovedResponse, CanisterIds, FinancialDetails, FormMetadata,
    MarketDetails, Metadata, PropertyDetails, SaleData, SaleStatus, Status,
};
...
pub struct Config {
    pub authorised_principals: HashSet<Principal>,
}
...
#[update]
fn add_known_principals(new_admin: Principal) -> Result<String, String> {
...
#[query]
fn verify_admin(caller: Principal) -> bool {
    let principal_list =
        CANISTER_DATA.with(|canister_data| canister_data.borrow().known_principals.to_owned());
...
#[update]
fn remove_known_principals(admin_to_remove: Principal) -> Result<Vec<Principal>, String> {
    if !is_controller(&caller()) {
        return Err("UnAuthorised Access".into());
    }
...
#[update(guard = "caller_is_authorized_principal")]
fn add_authorised_principal(new_principal: Principal) -> Result<String, String> {
    // let caller = caller();
    // if caller
...
#[query]
fn get_authorised_principal() -> Result<HashSet<Principal>, String> {
    // let caller = caller();
    // if caller
    //     != Principal::from_text("v3mpp-bismc-wjug7-6t6jc-iqu2b-64xh3-rpwld-sw5e2-fsecm-6lfss-aqe")
    //         .unwrap()
...
#[update]
fn update_key(new_str: String) -> Result<String, String> {
    if !is_controller(&caller()) {
        return Err("UnAuthorised Access".into());
    }
...
#[update]
fn init_minter_wasm(wasm: Vec<u8>) -> Result<String, String> {
    if !is_controller(&caller()) {
        return Err("UnAuthorised Access".into());
    }
...
#[update]
fn init_asset_wasm(wasm: Vec<u8>) -> Result<String, String> {
    if !is_controller(&caller()) {
        return Err("UnAuthorised Access".into());
    }
...
#[update]
fn get_form_metadata(index: u16) -> Result<FormMetadata, String> {
    CANISTER_DATA.with_borrow(|canister_data| {
        let form_data = canister_data
            .form_data
            .get(&index)
            .cloned()
            .ok_or("no data for this index".to_string())?;
        Ok(form_data)
    })
}
...
async fn approve_collection(index: u16, approval: bool) -> Result<ApprovedResponse, String> {
    ...
    let form_data = form_list.get(&index).ok_or("no form data for the index")?;
    ...
    if approval {
        ...
        let (canister_id_1,): (CanisterIdRecord,) =
            create_canister(create_arg.clone(), 200_000_000_000)
                .await
        ...
        let serialized_bytes: Vec<u8> = match install_arg {
            ...
            Some(install_args) => {
                candid::encode_args((install_args,)).expect("Failed to encode arguments")
            }
        ...
        CANISTER_DATA.with_borrow_mut(|canister_data| {
            ...
            canister_data
                .canister_store
                .insert(minter_canister_id.clone(), canister_id_data.clone());
...
#[update]
fn init_form_metadata(form_input: FormMetadata) -> Result<String, String> {
    ...
    CANISTER_DATA.with_borrow_mut(|canister_data| {
        ...
        canister_data
            .form_data
            .insert(canister_data.form_counter, form_data);
...
//collection specific data
#[update]
fn get_form_list() -> BTreeMap<u16, FormMetadata> {
